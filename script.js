...
function render(){
  ...
  window._timetableRefreshTimer = setTimeout(() => {
    ...
    const totalTracks = Object.keys(timeSlotMap.reduce((acc, slot) => {
      if (!slot) return acc;
      Object.keys(slot).forEach(t => acc[t] = true);
      return acc;
    }, {})).length;
    
    // Second pass: create the class cards with correct widths
    sortedDayClasses.forEach((item, classIndex) => {
      const [startHour, startMin] = item.startTime.split(':').map(Number)
      const [endHour, endMin] = item.endTime.split(':').map(Number)
      ...
      // Convert to pixels: 1 minute = 1px (60px per hour)
      const startPos = startOffsetInMinutes
      const endPos = endOffsetInMinutes
      const height = endPos - startPos
      
      // Get the specific day date for this column to check exceptions
      const checkDay = getHKTDate(weekStart)
      checkDay.setDate(weekStart.getDate() + dayIndex)
      const dateStr = formatHKTDateString(checkDay)
      ...
      // Set appropriate class names
      card.className = `class-card time-positioned${isCurrent ? ' current-class' : ''}${isException ? ' exception-class' : ''}`
      card.style.top = startPos + 'px'
      card.style.height = Math.max(height, 30) + 'px' // Minimum height for readability
      
      // Set width and left position based on total tracks
      // With CSS Grid, we can use simpler positioning
      if (totalTracks > 1) {
        // Multiple overlapping classes - distribute them horizontally
        const trackWidth = 100 / totalTracks;
        card.style.width = `calc(${trackWidth}% - 2px)`;
        card.style.left = `calc(${track * trackWidth}%)`;
      ...
      let tooltipText = `${item.title} (${item.crn})\nLocation: ${item.location}\nTime: ${formatTimeRange(item.startTime,item.endTime)}\nProfessor: ${item.profName}`
      if (item.profEmail) tooltipText += `\nProf Email: ${item.profEmail}`
      if (item.contactEmail) tooltipText += `\nContact Email: ${item.contactEmail}`
      tooltipText += `\nDays: ${item.weekdays ? item.weekdays.join(', ') : 'N/A'}\nPeriod: ${item.startDate} to ${item.endDate}`
      if (item.exceptions && item.exceptions.length > 0) {
        tooltipText += '\nExceptions: ' + item.exceptions.join(', ')
      }
      if (item.groupName) {
        tooltipText += `\nGroup: ${item.groupName}`
        if (item.groupMembers && item.groupMembers.length > 0) {
      ...
      card.innerHTML = `
        <div class="class-content">
          ...
          <div class="class-location">${escapeHtml(item.location)}</div>
          ${isException ? '<div class="exception-note">No Class This Week</div>' : ''}
          ${height > 60 ? `<div class="class-meta">Prof: ${escapeHtml(item.profName)}</div>` : ''}
        </div>
      `
      
      // Add single click functionality for popup
      ...
      // Add touch/click event listeners for mobile
      card.addEventListener('touchend', (e) => {
        e.preventDefault()
        showClassPopup(item)
      })
      
      // Add long press and right click functionality for detail page
      addLongPressListener(card, () => showClassDetails(item))
